...
export async function createShare(fileId: string, password: string | null, expiresAt: Date | null, downloadLimit: number | null): Promise<ShareRecord> {
  const sql = getSql();
  const id = generateId();
  const passwordHash = password ? await hashPassword(password) : null;
  const rows = (await sql`
    INSERT INTO shares (id, file_id, password_hash, expires_at, download_limit)
    VALUES (${id}, ${fileId}, ${passwordHash}, ${expiresAt?.toISOString() || null}, ${downloadLimit})
    RETURNING *
  `) as unknown[];
...
export async function validateShare(shareId: string, password: string | null): Promise<ShareRecord | null> {
  const sql = getSql();
...
export async function incrementShareDownloadCount(shareId: string): Promise<void> {
  const sql = getSql();
  await sql`UPDATE shares SET download_count = download_count + 1 WHERE id = ${shareId}`;
}
...
export async function createApiKey(name: string, permissions: string, expiresAt: Date | null): Promise<{ id: string; key: string; key_hash: string }> {
  const sql = getSql();
  const id = generateId();
  const rawKey = `ld_${generateId()}${generateId().replace(/-/g, '')}`;
  const keyHash = await hashPassword(rawKey);
  const rows = (await sql`
    INSERT INTO api_keys (id, name, key_hash, permissions, expires_at)
    VALUES (${id}, ${name}, ${keyHash}, ${permissions}, ${expiresAt?.toISOString() || null})
    RETURNING *
  `) as unknown[];
  return { id: rows[0] as ApiKeyRecord, key: rawKey };
}
...
export async function createAuditLog(action: string, details: string | null = null, adminId: string | null = null, ipAddress: string | null = null, userAgent: string | null = null): Promise<void> {
  const sql = getSql();
  const id = generateId();
  await sql`
    INSERT INTO audit_log (id, admin_id, action, details, ip_address, user_agent)
    VALUES (${id}, ${adminId}, ${action}, ${details}, ${ipAddress}, ${userAgent})
  `;
}

export async function listAuditLogs(limit = 100, offset = 0): Promise<AuditLogRecord[]> {
  const sql = getSql();
